...
public class EjecutorPython {
	...
	public String ejecuta2(String fileName) {
	    // Ruta al entorno virtual
	    String venvPath = "D:\\DiseñoSofware2024\\ocean";
	    ...
	    // Crea un ProcessBuilder para cada comando
	    ProcessBuilder activatePB = new ProcessBuilder("powershell", "/c", activateCommand);
	    ProcessBuilder pythonPB = new ProcessBuilder("powershell", "/c", pythonCommand);
	    ProcessBuilder deactivatePB = new ProcessBuilder("powershell", "/c", deactivateCommand);

	    // Establecer el PATH y otras variables de entorno
	    Map<String, String> env = pythonPB.environment();
	    env.put("PATH", env.get("PATH") + File.pathSeparator + venvPath);

	    // Redireccionar la salida estándar de la consola a un archivo
	    String carpeta = fileName.substring(0, fileName.length()-9);
	    String salida = carpeta + "salida.txt";
	    String errores = carpeta + "errores.txt";
	    pythonPB.directory(new File(carpeta));
	    pythonPB.redirectOutput(new File(salida));
	    pythonPB.redirectError(new File(errores));

	    try {
	        // Activa el entorno virtual
	       
	        pythonPB = new ProcessBuilder("powershell", "/c", activateCommand);
	        Process activateProcess = pythonPB.start();
	        activateProcess.waitFor();

	        // Ejecuta el script de Python
	        pythonPB = new ProcessBuilder("powershell", "/c", pythonCommand);
		    pythonPB.directory(new File(carpeta));
			pythonPB.redirectOutput(new File(salida));
			pythonPB.redirectError(new File(errores));
	        activateProcess = pythonPB.start();
	        pythonPB.redirectOutput(new File(salida));
	        activateProcess.waitFor();

	        // Desactiva el entorno virtual
	        pythonPB = new ProcessBuilder("powershell", "/c", deactivateCommand);
	         activateProcess = pythonPB.start();
	        activateProcess.waitFor();

	        // Leer el archivo de salida y el archivo de errores
	        if(new File(errores).length()>0) {
	            // Si hay error, devolver a quien corresponda el mensaje de error
	            return errores;
	        }
	        else {
	            // Si no hay error, devolver a quien corresponda el resultado de la salida
	            String respuesta;
	            try (FileInputStream fis = new FileInputStream(new File(salida))) {
	                byte[] bytes = new byte[fis.available()];
	                fis.read(bytes);
